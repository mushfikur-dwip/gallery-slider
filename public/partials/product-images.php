...
if ( ! class_exists( 'WCGS_Product_Gallery' ) ) {
	...
	/**
	 * Add ACF gallery images for custom post types
	 */
	private function add_acf_gallery_images() {
		if ( ! class_exists( 'WCGS_Public_Acf' ) ) {
			return;
		}

		$post_type  = get_post_type( $this->product_id );
		$field_name = WCGS_Public_Acf::get_field_name_for_post_type( $post_type );
	...
	public function build_gallery() {
		try {
			// Check if this is a custom post type with ACF gallery support.
			$post_type = get_post_type( $this->product_id );
			if ( class_exists( 'WCGS_Public_Acf' ) && WCGS_Public_Acf::is_supported_post_type( $post_type ) ) {
				$this->add_acf_gallery_images();
			} elseif ( $this->product ) {
			...
			if ( empty( $this->gallery ) ) {
				if ( $this->product ) {
					...
					$image_id = get_post_thumbnail_id( $this->product_id );
				}
				if ( $this->validate_image_id( $image_id ) ) {
					$this->gallery[] = $this->helper->wcgs_image_meta( $image_id, $this->settings );
				}
