...
class Woo_Gallery_Slider_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		// Add gallery to Car post type single pages.
		add_filter( 'the_content', array( $this, 'wcgs_display_car_gallery' ), 20 );
		add_action(
	...
	/**
	 * Display gallery for Car post type.
	 *
	 * @param string $content Post content.
	 * @return string
	 */
	public function wcgs_display_car_gallery( $content ) {
		if ( ! is_singular( 'car' ) || ! in_the_loop() || ! is_main_query() ) {
			return $content;
		}

		// Check if ACF plugin and car_gallery field exist.
		if ( ! class_exists( 'WCGS_Public_Acf' ) || ! WCGS_Public_Acf::has_gallery( get_the_ID(), 'car_gallery' ) ) {
			return $content;
		}

		// Generate gallery HTML.
		$gallery = $this->wcgs_woocommerce_show_product_images();

		// Prepend gallery before content.
	...
	public function enqueue_scripts() {
		 ...
		 * class.
		 */
		if ( is_singular( 'product' ) || is_singular( 'car' ) ) {
			global $post;
			$product_id = $post->ID;
			
			// Get product or post data based on post type.
			if ( is_singular( 'product' ) ) {
			...
			// Update the json_data (only for WooCommerce products with variations).
			if ( is_singular( 'product' ) && isset( $product ) ) {
